       ...
       <div class="Picking">
           ...
           <el-col :span="12" class="isLeft">
               ...
               <el-table
                   ...
                   @current-change="handleCurrentSelectionChange1"
                   style="width: 100%;">
                   <el-table-column
                       type="selection"
                       width="55">
                       ...
                       <template scope="scope">
                           <div class="flex2">
                               ...
                               <el-popover
                                   ...
                                   width="200"
                                   trigger="hover"
                                   :content=scope.row.productCode>
                               </el-popover>
                               <p style="display:block;white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" v-popover:popover1>{{scope.row.productCode}}</p>
                           </div>
                       </template>
                   </el-table-column>
                   <el-table-column label="颜色/尺寸">
                       <template scope="scope">{{scope.row.color + "/" + scope.row.size}}</template>
                   </el-table-column>
                   <el-table-column
                       prop="unitPrice"
                       label="单价">
                   ...
                   <el-table-column
                       ...
                       label="配货数量">
                       <template scope="scope">
                           <el-input type="number" size="small" v-model="scope.row.packedNum" :min="0" :max="scope.row.unPackedNum"  onkeypress="return event.keyCode>=48&&event.keyCode<=57&&scope.row.packedNum<=scope.row.unPackedNum"></el-input>
                       </template>
                   </el-table-column>
               </el-table>
           ...
           <el-col :span="12" class="isLeft">
               ...
               <ul>
                   <li>
                       <div class="change3">
                            <img src="../image/parcel03.png" alt="">
                       </div>
               ...
               <el-table
                   ...
                   <el-table-column
                       type="selection"
                       width="55"
                       v-if="currPackageState<1">
                   </el-table-column>
                   <el-table-column
                       label="主图/货号">
                       <template scope="scope">
                           <div class="flex2">
                           	   <img v-if="scope.row.imgUrl_main != undefined&&scope.row.imgUrl_main != ''" :src="scope.row.imgUrl_main" alt=""  class="tableImg">
                               <img v-else src="../image/product_default.png" alt=""  class="tableImg">
                               <el-popover
                                   ref="popover1"
                                   placement="top"
                   ...
                   <el-table-column
                    ...
                    <el-table-column
                       ...
                       label="移除数量">
                       <template scope="scope">
                           <el-input type="number" size="small" v-model="scope.row.moveNum" :min="0" :max="scope.row.containProductNum"  v-show="currPackageState < 1" onkeypress="return event.keyCode>=48&&event.keyCode<=57"></el-input>
                       </template>
                   </el-table-column>
               </el-table>
               <div class="flex2 leftButton">
...
<script>
    ...
    import {findOrderDetail,queryPackageList,savePackage,queryPackage,deletePackage,endOfPacked} from '@/api/getData'
    ...
    export default {
        ...
        methods: {
            ...
            handleCurrentSelectionChange1(val){
            ...
            async queryPackage(){
                ...
                const res = await queryPackage(params);
                if(res.isSuccess){
                    let para = {
                        list:'-1',
                        name:'新建包裹',
            ...
            batchAddToList(){
                ...
                for(var i = 0 ;i < this.multipleSelection.length;i++){
                    if(this.multipleSelection[i].packedNum<=0){
                        continue;
                    }
            ...
            batchMoveOutOfPackage(){
                ...
                for(var i = 0;i<this.multipleSelection2.length;i++){
                    if(this.multipleSelection2[i].moveNum<0){
                        continue;
                    }
                    if(Number(this.multipleSelection2[i].moveNum) > Number(this.multipleSelection2[i].containProductNum)){
                        flag = true;
                        break;
                    }
                    array.push(this.multipleSelection2[i]);
                }
                if(flag){
                    this.$message({
                        type: 'error',
                        message: '移除数量不能大于配货数量，请检查！'
                    });
                    return;
                }
                if(array.length <1){
...
//                    this.$router.push({path:"/indentManagementPortionPicking",query:{orderId:this.orderId,orderCode:this.orderCode,busiCompId:this.busiCompId,status:this.status,busiName:this.busiName,orderType:this.orderType,orderTime:this.orderTime,totalFee:this.totalFee,totalNum:this.totalNum}});
            ...
            async tabs(index){
                this.currPackageState = this.packageList[index].state;
