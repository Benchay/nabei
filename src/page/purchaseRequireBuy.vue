    ...
    <div class="fillcontain">
        ...
        <div class="startProcurement">
            ...
            <div class="orderTop flex">
                ...
                <div>
                    <el-button class="buttonColor3" @click="showSearchProduct" :disabled="unfinished">手动添加</el-button>
                </div>
                <div class="uploatfile">
                    <el-button class="buttonColor3" @click="dialogImport = true" :disabled="unfinished">导入需求</el-button>
                    <!-- <input type="file"  accept=".xls" id="uploatfile" @change="importf" v-if="accomplish"/> -->
                </div>
                <div>
                    <el-button class="buttonColor3" @click="partdelete" :disabled="unfinished">批量删除</el-button>
                </div>
            </div>
            ...
            <div>
                <div class="salesOrderTable">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableFirst">
                        <tr>
                            <td>
                                ...
                                <div style="text-align: left; padding-left: 18px"> <input type="checkbox" v-model="checked"   class="checkbox mr-10" @click="selectAll"/>主图/货号</div>
                            </td>
                            <td>颜色</td>
                            <td>尺码</td>
                            <td>单价</td>
                    ...
                    <div v-if="radio==2" class="tableHead">
                       ...
                       <table width="100%" border="0" cellpadding="0" cellspacing="0" class="buyTable">
                           <tr>
                               <th colspan="3" class="red flex unfinisdhedFirst" style="padding-left: 0;"><span style="width: 80px; padding-left: 18px;">未匹配到档口</span><img src="../image/unregistered_79.png" alt=""></th>
                              <!-- <th>采购数量：15</th>
                               <th>总金额：3200.00</th>-->
                           </tr>
                       </table>
                       <!--表格内容-->
                       <div class="tableContent2" v-for="items in tableNoStall">
                           <table width="100%" border="0" cellpadding="0" cellspacing="0" class="buyTable2">
                               <tr v-for="item in items.purchases">
                                   <td >
                                       <div class="tdFirst flex">
                                       	   <img v-if="item.picPath != ''" class="buyImg1" :src="item.picPath" alt="">
                                           <img v-else class="buyImg1" src="../image/product_default.png" alt="">
                                   ...
                                   <td>
                                       <div class="tdLast flex2">
                                           <a href="javascript:;" @click="delePurchase(item.id)"  class="red">删除</a>
                                       </div>
                                   </td>
                               </tr>
                               <!--
                    ...
                    <div v-if="radio==0 || radio==1 " ass="tableHead">
                        <!--表头-->
                        <div v-for="(item,index) in tableDataHad">
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="buyTable">
                            <tr>
                                <th>
                                   <!-- <el-checkbox class="left salesCheckbox"  v-model="selectCheckbox" :value="item.stallId" >{{item.stallName}}</el-checkbox>-->
                                    <input type="checkbox" class="mr-10" :value="item.stallId"   @change="partAllSelect(item)"  v-model="item.checked"  >{{item.stallName}}
                                </th>
                                <th></th>
                                <th></th>
                                <th></th>
                                ...
                                <tr v-for="subitem in item.purchases">
                                    <td>
                                        <div class="tdFirst">
                                            <!--s是否关联商品-->
                                            <div>
                                            <input type="checkbox"  v-model="subitem.checked"      @change="singleSelect(item)"/>
                                            </div>
                                            ...
                                            <!---->
                                            <img v-if="subitem.picPath != ''" class="buyImg1" :src="subitem.picPath" alt="">
                                            <img v-else class="buyImg1" src="../image/product_default.png" alt="">
                                            <div >{{subitem.productCode}}</div>
                                            <sup style="margin-left: 15px;" v-if="subitem.saleState == 0"><img src="../image/downed.png" alt=""></sup>
                                        </div>
                                    </td>
                                    <td>{{subitem.color}}</td>
                                    <td>{{subitem.size}}</td>
                                    ...
                                    <td>
                                        <div class="tdLast flex" style="padding-left: 75px; white-space: nowrap;">
                                            <a href="javascript:;"  @click="delePurchase(subitem.id)" class="red">删除</a>
                ...
                <a href="javascript:void(0)" class="export_excle1" @click="export2Excel">导出excel</a>
            </div>
           ...
           <div class="newRequirement radioRed">
              ...
              <div v-if="commodityInformation">
                  <el-col :span="24" class="flex newList">
                      ...
                      <el-col :span="18" class="content">
                          <img v-if="imgUrl_main != ''" style="width: 160px; height: 160px;" :src="imgUrl_main" alt="" class="newRequirementImg">
                          <img v-else style="width: 160px; height: 160px;" src="../image/product_default.png" alt="" class="newRequirementImg">
                      </el-col>
                  </el-col>
                  <el-col :span="24" class="flex newList">
                      <el-col :span="5" class="title">颜色：</el-col>
                      <el-col :span="18" class="content">
                          <el-radio-group v-model="color" size="small" @change="selectColor">
                              <el-radio-button :label="item" v-for="item in colors">{{item}}</el-radio-button>
                          </el-radio-group>
                      </el-col>
                  </el-col>
                  <el-col :span="24" class="flex newList">
                      ...
                      <el-col :span="18" class="content">
                          <el-radio-group v-model="size" size="small" @change="selectSize">
                              <el-radio-button :label="item" v-for="item in sizes">{{item}}</el-radio-button>
                          </el-radio-group>
                      </el-col>
                  </el-col>
                  <el-col :span="24" class="flex newList">
...
<script>
    ...
    import {querySellerProduct,queryPurchaseRequire,importPurchaseReauireOrder,deletePurchaseRequire,createOrderByPurchaseRequire,
    	saveOrUpdatePurchaseRequire,querySellerProductsByRelationId,asyncStallCustomer,queryCompany,batchDeletePurchaseRequire,loadSellerProductInfo} from '@/api/getData'
...
//                是否关联
        ...
        methods: {
            ...
            /*批量删除*/
            async partdelete(){
                console.log(this.selectOrderString)
                let param ={
                purchaseRequireId:this.selectOrderString,
            }
            ...
            let Count=0;
            let num=0;
            arr.purchases.forEach(function(item){
                if(item.checked){  Count++;  }
                Count==arr.purchases.length?arr.checked=true:arr.checked=false;
            });

            _this.tableDataHad.forEach(function(item){
                if(item.checked){   num++; }
                num==_this.tableDataHad.length?_this.checked=true:_this.checked=false;
            }) ;
            ...
            partAllSelect(item){
                ...
                item.purchases.forEach(function(items, index) {
             ...
             /*所有店铺的全选按钮*/
              selectAll() {
                ...
                this.tableDataHad.forEach(function(item){
            ...
            async searchPurchaseProduct(){
                ...
                this.productId = '';
                this.options4.forEach(obj => {
                	if(obj.productCode == this.productCode){
               ...
               //搜索商品货号
                ...
                const res = await loadSellerProductInfo(param);
                if(res.isSuccess){
                	this.sellerProductInfo = res.result;
                	if(res.result.saleState && res.result.saleState == 1){
                		if(res.result.imgUrl_main){
                		...
                		this.customerName = res.result.companyName;
                		this.commodityInformation = true;
                	}else{
                		this.$message({
                            type:'error',
            ...
            selectColor(){
                ...
                this.productVariant.forEach( obj => {
                	if(obj.colour == this.color){
                		this.sizes.push(obj.size);
                	}
                });
            },
            selectSize(){
            	this.productVariant.forEach( obj => {
                	if(obj.colour == this.color && obj.size == this.size){
            ...
            async addPurchaseProductInfo(){
                ...
                if( this.productId==undefined||this.productId==''||this.productVariantId==undefined||this.productVariantId==''||this.size == ''){
                    this.$message({
                        duration:this.GLOBAL.MESSAGE_DISSMISS_TIME_MID,
                        type:'error',
                        message:'请选择商品规格和数量'
                    ...
                    if(res.result.data != null){
                        var data= res.result.data.results;
                               data.forEach(function(item){
                                   item['checked']=false;
                                   item.purchases.forEach(function(item){
                                         item['checked']=false;
                                   });
                               });
            ...
            * */
            ...
            async findOrderNoStall(){
                ...
                const res = await queryPurchaseRequire(param);
                if (res.isSuccess == true) {
                    this.tableNoStall = res.result.data.results;
                    this.totalCount = res.result.data.totalCount;
                }else{
                    this.$message({
                        type: 'error',
            ...
            async addOrder(){		//生成采购订单
               ...
               // var purchaseIds=linkArrayFormatId(showData.idsArray);
                ...
                let param = {
                    purchaseIds:this.selectOrderString,
                    model:1,
                    companyId:this.roleForm.compId
                }
		...
		　　　},
            ...
            async showStartPurchase(){
                let checkNum=0; // 所有的数量
                let orderNum=0; // 订单数量

                let totallPrice=0;// 总价
                let selectOrderArr=[];
               this.tableDataHad.forEach(function(item){
                      item.purchases.forEach(function(items, index) {
                          if(items.checked){
                              orderNum+=1;
                              checkNum+=items.requireCnt;
                              if(items.specCompany){
                                  totallPrice+=items.price*items.requireCnt;
                              }else{
                                  totallPrice+=items.currPrice*items.requireCnt;
                              }
                              selectOrderArr.push(items.id);
                          }
             ...
             if(!checkNum){
                ...
                this.selectOrderString=selectOrderArr.join(',');
                if(!totallPrice){
                    this.show_totalFeeWillPay=totallPrice;
                }else{
                    this.show_totalFeeWillPay=totallPrice.toFixed(2);
                }
                this.show_totalNum=checkNum;
                this.show_orderNum=orderNum;
            ...
            let selectOrderArr=[];
               ...
               let idarray = selectOrderArr.join(',');
                let param = {
                    ids:idarray
                }
                const res=await batchDeletePurchaseRequire(param);
                if (res.isSuccess){
                    this.initloadData();
                    this.$message({
                        type: 'success',
