    ...
    <div class="fillcontain">
        ...
        <div class="sellerSettlement">
            ...
            <div class="details">
                ...
                <el-col :span="24">
                    <el-col :span="6">当前总欠款金额：{{this.orderVo.totalFee==undefined||this.orderVo.totalFee==0?0:Number(this.orderVo.totalFee).toFixed(2)}}</el-col>
                    <el-col :span="6">已结金额：{{this.orderVo.closeFee==undefined||this.orderVo.closeFee==''?0:Number(this.orderVo.closeFee).toFixed(2)}}</el-col>
                    <el-col :span="6" class="colorText">待结金额：{{this.orderVo.closeFee==undefined?Number(this.orderVo.totalFee).toFixed(2):Number(this.orderVo.totalFee - this.orderVo.closeFee).toFixed(2)}}</el-col>
                    <!--已结算状态 无此按钮-->
                    <el-col :span="6">
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template scope="scope">
                            {{scope.row.orderFee!==undefined&&scope.row.orderFee!=''?Number(scope.row.orderFee).toFixed(2):0}}
                        </template>
                    </el-table-column>
                    <el-table-column
                        prop="type"
                    ...
                    <el-table-column
                        ...
                        label="实际付款金额（￥）">
                        <template scope="scope">
                            {{scope.row.sellementFee!==undefined&&scope.row.sellementFee!=''?Number(scope.row.sellementFee).toFixed(2):0}}
                        </template>
                    </el-table-column>
                    <el-table-column
                        prop="status"
                    ...
                    <el-table-column
                        label="操作">
                        <template scope="scope">
                            <router-link :to='{path:"/indentManagementAccountDetails",query:{settlementOrderId:scope.row.settlementOrderId,currentFee:Number(scope.row.orderFee).toFixed(2),backPath:backPath2}}' class="blue2">查看详情</router-link>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //立即付款
            quickSettlement(){
                this.$router.push({path:'/sellerSettlementImmediately',query:{actualPayFee:Number(Number(this.orderVo.totalFee) - Number(this.orderVo.closeFee)).toFixed(2),backOrderId:this.orderId,customName:this.orderVo.busiCompanyName,activeName:"second",saleCompanyId:this.orderVo.busiCompId,payPath:this.backPath2}});
            },
