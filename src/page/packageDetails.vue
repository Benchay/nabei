<template>
 ...
 -->                        <!--
                ...
                <div class="orderTable">
                    <el-table
                        ...
                        <el-table-column
                            ...
                            label="状态">
                            <template scope="scope">
                                <p v-bind:class="[scope.row.currPackageLogState<22?'red':'']">{{scope.row.currPackageLogState==-1?'空包裹':scope.row.currPackageLogState==0?'待拿货':scope.row.currPackageLogState==1?'待装车':scope.row.currPackageLogState==2?'待卸货':scope.row.currPackageLogState==3?'已完成':scope.row.currPackageLogState==21?'待入库':''}}</p>
                            </template>
                        </el-table-column>
                         <el-table-column
                            prop="packageType"
        ...
        <el-dialog
            ...
            <a href="javascript:void(0)" @click="addPack1" class="buttonColor3">确 定</a>
    ...
    export default {
        ...
        methods: {
            ...
            async addpack(bprint,btest){
                if(!this.packnum || isNaN(this.packnum)){
                        this.$message({
                            type: 'error',
                            message: "请输入正确的包裹数量"
                        });
