...
<script>
    import {createByReturnRequire,queryReturnRequire} from '@/api/getData'
...
//                    console.log(res.result.results);
             ...
             async returnsAffirm(){
                ...
                this.checkedStall.forEach((obj1,index1)=>{//选中的档口Id数组
                        this.tableData.forEach((obj2,index2)=>{//所有的档口数组
                            if(obj1 == obj2.stallId){//如果选中档口id和数组中的某一个元素的档口id相等
                                let reason = '';
                                this.options.forEach((obj3,index3)=>{//原因数组
                                    if (obj3.value == this.valueData[index2] ){//取原因label
                                        reason = obj3.label;
                                    }
                                });
                                let item = {
