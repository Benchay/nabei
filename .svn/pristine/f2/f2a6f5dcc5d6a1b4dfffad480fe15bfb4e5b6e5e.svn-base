 ...
 */
import {getMenuAuthSort} from '../utils/AuthMenu'
...
export function getMenuAuth(resdata) {
    ...
    for (var i = 0; i < resdata.length; i++) {
        ...
        if (resultData.type == '1' && resultData.parentAuthId == null && resultData.isShowed == "1") { //一级菜单
            var json = new Object;
    ...
    for (var i = 0; i < jsonArray1.length; i++) {
		...
		//when no auth in parents menu,but had submenu
		jt1.index=getMenuAuthSort(jt1.index,jt1.id,jsonArray2);
