                    ...
                    <el-tab-pane label="发起收款" name="first" v-if="activeName == 'first'">
                        <div class="gathering">
                            ...
                            <el-col :span="12" class="flex">
                                ...
                                <el-col :span="18"  class="tkList">
                                    <el-input type="text" v-model="firstActualPayFee" placeholder="请输入" class="input1" @change="checkFirstActualPayFee" :disabled="isEdit"></el-input>
                                </el-col>
                            </el-col>
                            <el-col :span="12" class="flex">
...
<script>
    import {getSettlementOrder,queryPlatformAccount,createOffLineFinanceRecord,payForSettlementOrder,saveSettlementOrder,getOrder} from '@/api/getData'
    ...
    export default {
        ...
        watch:{
            '$route': function (route) {
                ...
                if(this.settlementOrderId!=undefined&&this.settlementOrderId!=''){
                ...
                }else if((this.orderId!=undefined&&this.orderId!='')||(this.orderIds!=undefined&&this.orderIds.length >0)){
                    this.loadOrderInfo();
                }else{
                    ...
                    if(this.actualPayFee!=undefined&&this.actualPayFee!=''){
                        this.actualPayFee = this.getRoundNumbers(this.actualPayFee);
                    }else{
                        this.actualPayFee = 0.00;
                    }
                    if(this.totalDebtFee!=undefined&&this.totalDebtFee!=''){
                        this.totalDebtFee = this.getRoundNumbers(this.totalDebtFee);
                    }else{
                ...
                if(sId!=undefined&&sId!=''){
                    this.saleOrderId = this.getRoundNumbers(sId);
                    this.initData();
                }
            }
        },
        ...
        methods: {
            ...
            checkFirstActualPayFee(){
                this.$nextTick(() =>{
                    var pattern = /^-?\d+\.?\d{0,2}$/;

                    if(this.firstActualPayFee==undefined||this.firstActualPayFee===''){

                    }else if(!pattern.test(this.firstActualPayFee)){
                        var str = this.firstActualPayFee;
                    ...
                    if(Number(this.firstActualPayFee)>Number(this.totalFee)||Number(this.firstActualPayFee)<=0){
                        this.firstActualPayFee = this.totalFee;
                    }
                });
            },
                ...
                if(this.orderId!=undefined&&this.orderId!=''){
                    ...
                    if(res.isSuccess){
                        ...
                        if(orderVo.closeFee==undefined||orderVo.closeFee==''){
                            this.actualPayFee = this.getRoundNumbers(orderVo.totalFee);
                        }else{
                            this.actualPayFee = this.getRoundNumbers(orderVo.totalFee*1 - orderVo.closeFee*1);
                        }
                        this.payFeeTemp = this.actualPayFee;
                    }
                    ...
                    for(var i = 0;i<this.orderIds.length;i++){
                        ...
                        const res2 = await getOrder(par);
                        if(res2.isSuccess){
                            var closeFee = 0;
                            if(res2.result.closeFee==undefined||res2.result.closeFee==''){
                                closeFee = 0;
                            }else{
                                closeFee = res2.result.closeFee;
                            }
                            this.actualPayFee = this.actualPayFee + Number(Number(res2.result.totalFee) - Number(closeFee));
                            this.actualPayFeeTemp = this.actualPayFee;
                        }
                    }
    ...
    //-------------------------------发起收款部分--------------------------------
            //查询多个订单金额
            async loadSaleReturnOrdersInfo(){
                ...
                for(var i = 0 ; i < this.backOrderIds.length;i++){
                    let param = {
                        "id": this.backOrderIds[i],
                        "getOrderId":1
                    };
                    const res = await getOrder(param);
                    if (res.isSuccess) {
                        ...
                        this.totalFee = Number(this.totalFee) + Number(Number(vo.totalFee) - Number(vo.closeFee));
                        if(vo.closingFee==undefined){
                            vo.closingFee = 0;
                        }
                        this.firstActualPayFee = Number(this.firstActualPayFee) + Number(Number(vo.totalFee) - Number(vo.closeFee)-Number(vo.closingFee));
                    }
                }
                if(this.totalFee!=undefined&&this.totalFee!=''){
                    this.totalFee = this.getRoundNumbers(this.totalFee);
                }
                if(this.firstActualPayFee!=undefined&&this.firstActualPayFee!=''){
                    this.firstActualPayFee = this.getRoundNumbers(this.firstActualPayFee);
                }
                ...
                if(this.activeName == 'first'){
                    if(this.saleOrderId!=undefined&&this.saleOrderId!=''){
                        ...
                        if(res.isSuccess){
                            ...
                            if(data.closeFee==undefined){
                                data.closeFee = 0;
                            }
                            this.totalFee = Number(data.totalFee) - Number(data.closeFee);
                            this.totalFee = this.getRoundNumbers(this.totalFee);
                            this.firstActualPayFee = this.totalFee
                        }
                    }else{
