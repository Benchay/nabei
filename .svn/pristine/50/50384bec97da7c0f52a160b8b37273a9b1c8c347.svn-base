                ...
                <div style="overflow: hidden;width:100%;">
                    <el-col :span="24" class="basicBorder">
                        ...
                        <el-col :span="15" class="basicInfo">
                            <el-col :span="8"><span class="title">用户名：</span><span>{{this.roleForm.userName}}</span></el-col>
                            <el-col :span="16"><router-link :to='{path:"/changePassword"}' class="changepwd colorFM">修改密码</router-link></el-col>
                            <el-col :span="24"><span class="title">手机号码：</span><span>{{this.roleForm.mobile | toTel}}</span></el-col>
...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            var check=getSessionStore(types.SESSION_ADIN);
            if(check=='true'){
                this.adminFlage=check;
            }else{
                this.adminFlage=false;
            }
        ...
        methods: {
            ...
            async showEditCompany(data){
                ...
                if(codes.length==4){
	                ...
	                codes.splice(3,1);
	                this.editForm.location = codes.join('/');
                }
...
//                      	this.form.seralNub='';
            ...
            async querySearchAsync(queryString, cb) {
            	if(this.restaurants.length < 1){
        			const res = await queryUserByCompany({"compId": this.selectActiveCompanyId, "page": 1, "pageSize": 2000});
	                if (res.isSuccess == true) {
