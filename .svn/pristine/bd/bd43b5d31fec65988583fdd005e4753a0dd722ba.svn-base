...
export function getLodop(oOBJECT,oEMBED){
    ...
    var strHtmUpdate="打印控件需要升级!升级后请重新进入。</font>";
    var strHtm64_Install="打印控件未安装!安装后请刷新页面或重新进入。</font>";
    var strHtm64_Update="打印控件需要升级!升级后请重新进入。</font>";
    var strHtmFireFox="（注意：如曾安装过Lodop旧版附件npActiveXPLugin,请在【工具】->【附加组件】->【扩展】中先卸它）";
    var strHtmChrome="(如果此前正常，仅因浏览器升级或重安装而出问题，需重新执行以上安装）";
    var strCLodopInstall="CLodop云打印服务(localhost本地)未安装启动!执行安装后请刷新页面。";
    ...
    try{
        ...
        if (needCLodop()) {
	    	...
	    	if (!LODOP && document.readyState!=="complete") {
	    		result.code = 0;
	    		result.msg = "C-Lodop没准备好，请稍后再试！";
	    		return result;
	    	};
            if (!LODOP) {
				result.code = 0;
	    		result.msg = strCLodopInstall;
	    		return result;
            } else {
...
/* 	         if (CLODOP.CVERSION<"3.0.0.8") {
            ...
            if ((LODOP==null)||(typeof(LODOP.VERSION)=="undefined")) {
                 if (navigator.userAgent.indexOf('Chrome')>=0){
                 	result.code = 0;
		    		result.msg = strHtmChrome;
		    		return result;
                 }
                 if (navigator.userAgent.indexOf('Firefox')>=0){
                 	result.code = 0;
		    		result.msg = strHtmFireFox;
		    		return result;
                 }
                 if (is64IE){
                 	result.code = 0;
		    		result.msg = strHtm64_Install;
		    		return result;
                 }  else if (isIE){
                 	result.code = 0;
		    		result.msg = strHtmInstall;
		    		return result;
                 }else{
                 	result.code = 0;
		    		result.msg = strHtmUpdate;
		    		return result;
                 }
                 return LODOP;
            };
