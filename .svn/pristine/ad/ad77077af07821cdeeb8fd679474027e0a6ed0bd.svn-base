        ...
        <div class="purchasing-content">
            <div class="purchasingAll">
                ...
                <div class="orderDetails">
                    ...
                    <p>已结算金额：<span class="title">￥{{Number(alreadyFee).toFixed(2)}}</span></p>
                </div>
                ...
                <div v-show="nobao">
                    ...
                    <el-table
                        ...
                        <el-table-column
                            ...
                            <template scope="scope">
                                <ul>
                                    <li v-for="item in scope.row.details">{{item.orderNum}}</li>
                                </ul>
                            </template>
                        </el-table-column>
                        <el-table-column
                ...
                <div class="right"style="margin-top: 20px">
                    <div class="purPendingButton" v-show="remind">
                        <a href="javascript:void(0)" class="buttonColor3" type="info" size="small"  @click="packstate(clickIndex2)">包裹动态</a>
                    </div>
...
<script>
    ...
    import {queryPackage,queryOrderLog,sendRemind,getOrder,loadPackageInfo,findOrderDetail,queryPackageLogInfo} from '@/api/getData'
    ...
    export default {
        ...
        methods: {
            ...
            tabs(index){
                ...
                this.packCode = this.packageList[index].packCode;
            ...
            //包裹动态
            async packstate(index){
                this.dialog = true;
                this.packCode = this.packageList[index].packCode;
                let param = {
                    "packgeType":"0",
                    "packageCode": this.packCode,
                    "pageIndex":this.currentPage,
                    "pageSize":this.pageSize
                };
                // console.log(this.packageList);
...
//                }
            ...
            async sendMessage(){
                let param = {
                    "orderId":this.id,
                    "operate":"SELLER_URGE_ORDER"
                };
                const  res= await sendRemind(param);
                if(res.isSuccess){
                    this.$message({
                        type: 'success',
                        message: res.result
