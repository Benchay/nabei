            ...
            return {
                ...
                before:{
                    disabledDate: (time) => {
                        ...
                        var now = Date.parse(formatDate(new Date(),'yyyy-MM-dd 00:00:00'));
                        if (beginDateVal) {
                            if(beginDateVal > now){
                                return time.getTime() > beginDateVal || time.getTime() < now;
                            }else{
                                return time.getTime() < now ||  time.getTime() > beginDateVal;
                            }
                        }else{
