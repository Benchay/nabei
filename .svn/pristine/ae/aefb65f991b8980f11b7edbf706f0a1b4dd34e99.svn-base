<template>
    <div class="fillcontain">
        ...
        <div class="msgManagement">
            ...
            <a href="javascript:void(0)" class="export_excle" @click="exportData">导出excle</a>
        </div>
    </div>
</template>
			...
			async exportData() {
                ...
                if (res.isSuccess == true) {
                    res.result.data.forEach(obj => {
                    	let phone= '' ;//手机号
                    	let identifyingCode = '';//验证码
                    	let status = '';//发送状态
                    	let sendTime = '';//发送时间
                    	if(obj.phone){
                    		phone = obj.phone;
                    	}
                    	if(obj.identifyingCode){
                    	...
                    	if(obj.createTime){
                    		sendTime = formatDate(new Date(obj.createTime),'yyyy-MM-dd hh:mm:ss');
                    	}
                    ...
                    const tHeader = ['手机号', '验证码','发送状态','发送时间',];
