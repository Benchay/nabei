...
export function getPurchaseInfo(purData) {
    ...
    var orderNum = 0;
    var totalNum = 0;
    var totalFeeWillPay = 0;
    var idsArray = [];
    for (var i = 0; i < purData.length; i++) {
        var temp = purData[i];
        console.log(temp);
        //for(var x=0;x<temp.length;x++){
        totalFeeWillPay = totalFeeWillPay + Number(temp.totalFeeWillPay);
    ...
    return { "orderNum": orderNum, "totalFeeWillPay": totalFeeWillPay, "totalFee": totalFee, "totalNum": totalNum, "idsArray": idsArray };
}